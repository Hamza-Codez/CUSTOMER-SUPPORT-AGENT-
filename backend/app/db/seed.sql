...
insert into businesses (id, name, plan_tier) values
    ('biz_demo',  'Aeron Home Goods', 'core'),
    ('biz_other', 'Unrelated Seller', 'core')
on conflict (id) do nothing;

insert into customers (business_id, email, name) values
...
insert into orders (business_id, order_id, customer_id, status, placed_at, carrier, tracking_number, eta, total)
...
join customers c on c.business_id = v.business_id and c.email = v.email
on conflict (business_id, order_id) do nothing;

-- Line items. Quantities are what the tool reports as `item_count`.
insert into order_items (business_id, order_ref, product_name, qty, unit_price)
...
join orders o on o.business_id = v.business_id and o.order_id = v.order_id
where not exists (
